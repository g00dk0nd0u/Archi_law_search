...
def detect_schema(conn: sqlite3.Connection) -> dict[str, str]:
    ...
    return {
        ...
        "document_date_col": optional_matching_column(columns, ("document_date",)),
...
def run_like_search(
    ...
    return conn.execute(
        ...
        SELECT
            ...
            {coalesce_expr(schema.get("document_date_col", ""), "document_date")},
...
def run_fts_search(conn: sqlite3.Connection, schema: dict[str, str], terms: list[str], limit: int) -> list[sqlite3.Row]:
    ...
    return conn.execute(
        ...
        SELECT
            ...
            {coalesce_expr('n.' + schema.get("document_date_col", "") if schema.get("document_date_col", "") else '', "document_date")},
