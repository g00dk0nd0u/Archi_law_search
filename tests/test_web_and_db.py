...
class DatabaseAndWebTests(unittest.TestCase):
    ...
    def test_github_pages_normalizes_law_article_heading_line_breaks(self):
        ...
        self.assertIn("const formattedText = formatBodyText(rawText);", body_cell)
        self.assertIn('item.source === "law" ? normalizeLawArticleHeading(formattedText) : formattedText', body_cell)
        ...
        render_script = "\n".join(functions) + """
const terms = [];
const law = {source: "law", preview: "（目的） 第一条 preview"};
const notice = {source: "kokuji", preview: "（目的） 第一条 preview"};
process.stdout.write(JSON.stringify({
  lawPreview: bodyCellHtml(law, terms, false),
  lawFull: bodyCellHtml(law, terms, true, "（定義） 第三十五条の三 full body"),
  noticePreview: bodyCellHtml(notice, terms, false),
  noticeFull: bodyCellHtml(notice, terms, true, "（定義） 第三十五条の三 full body"),
}));
"""
        rendered = json.loads(
        ...
        self.assertIn("（目的）\n第一条 preview", rendered["noticePreview"])
        self.assertIn("（定義）\n第三十五条の三 full body", rendered["noticeFull"])
